...
- **Fetch**: 環境変数で指定された GitHub Projects にアクセスし、Item(Issue) の一覧をローカルにキャッシュする
- **Promote**: 昇格ルールに基づいて Item のステータスを昇格させる
...
| `GHPP_STATUS_INBOX` | inbox に対応する GitHub Projects のステータス名 |
| `GHPP_STATUS_PLAN` | plan に対応するステータス名 |
| `GHPP_STATUS_READY` | ready に対応するステータス名 |
| `GHPP_STATUS_DOING` | doing に対応するステータス名 |
| `GHPP_PLAN_LIMIT` | 計画フェーズで一度に昇格する上限数 |
